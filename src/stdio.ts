...
function fromPackageRoot(relative: string): string {
...
function scanCourseContent(): CourseState {
  const lessonDirs = fs
...
function mergeCourseStates(
  ...
  const merged = fresh.lessons.map((newL) => {
    ...
    if (steps.every((s) => s.status === 2)) status = 2;
    else if (steps.some((s) => s.status > 0)) status = 1;
...
function readCourseStep(lessonName: string, stepName: string): string {
  const lessonDirs = fs.readdirSync(courseDir);
  const lessonDir = lessonDirs.find(
    (d) => d.replace(/^\d+-/, "") === lessonName
  );
...
function readDocsContent(docPath: string): DocsResult {
  const fullPath = path.resolve(path.join(docsBaseDir, docPath));
  if (!fullPath.startsWith(path.resolve(docsBaseDir))) return { found: false };
  ...
  try {
    ...
    if (stats.isDirectory()) {
      ...
      if (dirs.length) {
        ...
        listing.push(
          ...dirs.map((d) => `- ${docPath ? `${docPath}/${d}` : d}`)
        );
        listing.push("");
      }
      if (files.length) {
        ...
        listing.push(
          ...files.map((f) => `- ${docPath ? `${docPath}/${f}` : f}`)
        );
        listing.push("");
      }
...
function searchDocs(query: string, baseDir: string): SearchResult[] {
...
const tools = [
  {
    ...
    description:
      "[🎓 COURSE] Start or resume the Circle Nanopayments course. Begins at the first lesson or picks up where you left off.",
    inputSchema: {
  ...
  },
  {
    ...
    description:
      "[🎓 COURSE] Get current course progress including completed lessons and steps.",
  ...
  },
  {
    ...
    description:
      "[🎓 COURSE] Advance to the next step in the current lesson.",
    inputSchema: { type: "object" as const, properties: {} },
  },
  {
    name: "clearCircleCourseHistory",
    description:
      "[🎓 COURSE] Reset all course progress. This cannot be undone.",
    inputSchema: {
      type: "object" as const,
      properties: {
        confirm: {
  ...
  },
  {
    name: "circleDocs",
    description:
      "[📚 DOCS] Browse Circle Nanopayments documentation by path. Request paths to explore available docs.",
    inputSchema: {
      type: "object" as const,
      properties: {
        paths: {
  ...
  },
  {
    ...
    description:
      "[📚 DOCS] Search across all Circle Nanopayments documentation.",
    inputSchema: {
      type: "object" as const,
      properties: {
...
async function handleTool(name: string, args: ToolArgs): Promise<string> {
  switch (name) {
    case "startCircleCourse": {
      ...
      if (!fresh.lessons.length) return "No course content found.";
      ...
      const lesson = state.lessons.find(
        (l) => l.name === state.currentLesson
      );
      if (!lesson) return "Error: current lesson not found.";

      const step = lesson.steps.find((s) => s.status !== 2);
      if (!step) {
        lesson.status = 2;
        const next = state.lessons.find(
          (l) => l.status !== 2 && l.name !== state.currentLesson
        );
        if (next) {
    ...
    case "getCircleCourseStatus": {
      ...
      const totalSteps = state.lessons.reduce(
        (s, l) => s + l.steps.length,
        0
      );
      const doneSteps = state.lessons.reduce(
        (s, l) => s + l.steps.filter((st) => st.status === 2).length,
        0
      );
    ...
    case "startCircleCourseLesson": {
      ...
      const step =
        target.steps.find((s) => s.status !== 2) || target.steps[0];
      if (!step) return `Lesson "${target.name}" has no steps.`;
    ...
    case "nextCircleCourseStep": {
      ...
      if (!lesson) return "Current lesson not found.";
      ...
      const nextIdx = lesson.steps.findIndex(
        (s, i) => i > curIdx && s.status !== 2
      );

      if (nextIdx !== -1) {
        lesson.steps[nextIdx].status = 1;
        saveCourseState(state);
        const content = readCourseStep(
          state.currentLesson,
          lesson.steps[nextIdx].name
        );
        return `🎉 Step "${lesson.steps[curIdx].name}" completed!\n\n📘 ${state.currentLesson}\n📝 Next: ${lesson.steps[nextIdx].name}\n\n${wrapContent(content)}`;
      }

      lesson.status = 2;
      const lessonIdx = state.lessons.findIndex(
        (l) => l.name === state.currentLesson
      );
      const nextLesson = state.lessons.find(
        (l, i) => i > lessonIdx && l.status !== 2
      );

      if (nextLesson) {
        state.currentLesson = nextLesson.name;
        if (nextLesson.steps[0]) nextLesson.steps[0].status = 1;
        nextLesson.status = 1;
        saveCourseState(state);
        const content = readCourseStep(
          nextLesson.name,
          nextLesson.steps[0]?.name ?? ""
        );
...
const pkg = JSON.parse(
  fs.readFileSync(fromPackageRoot("package.json"), "utf-8")
);

const server = new Server(
  { name: "Circle Nanopayments Documentation Server", version: pkg.version },
  { capabilities: { tools: {} } }
);
